...
// ── Slim variants (exclude content/notes/aiReview from bulk workspace load) ──

type CategoryWithSlimSolutions = Prisma.CategoryGetPayload<{
  include: {
    questions: {
      include: {
        solutions: {
...
type SlimSolutionRow = CategoryWithSlimSolutions["questions"][number]["solutions"][number];

function mapSlimSolution(solution: SlimSolutionRow): Solution {
  return {
    id: solution.id,
    questionId: solution.questionId,
    title: solution.title,
...
function mapSlimQuestion(question: CategoryWithSlimSolutions["questions"][number]): Question {
  return {
    id: question.id,
    categoryId: question.categoryId,
    title: question.title,
    ...
    createdAt: question.createdAt.toISOString(),
    updatedAt: question.updatedAt.toISOString(),
    solutions: [...question.solutions].sort((a, b) => a.order - b.order).map(mapSlimSolution),
    tags: question.tags.map(mapTag)
  };
}

function mapSlimCategoryRow(row: CategoryWithSlimSolutions, viewerUserId: string | null): Category {
  return {
    id: row.id,
    userId: row.userId,
    name: row.name,
    ...
    createdAt: row.createdAt.toISOString(),
    children: [],
    questions: [...row.questions].sort(compareQuestionsByPinAndOrder).map(mapSlimQuestion)
  };
}

export function buildSlimCategoryTree(rows: CategoryWithSlimSolutions[], viewerUserId: string | null): Category[] {
  const nodes = new Map(rows.map((row) => [row.id, mapSlimCategoryRow(row, viewerUserId)]));
  const roots: Category[] = [];
